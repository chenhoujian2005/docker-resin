...
<title>Simple jsp page</title>
    ...
    <script type="text/javascript">
        ...
        <c:if test="${sessionScope.userLogin.sfi.id eq 000}">
            ...
            thisForm.adjustApprove[0].checked = true;
...
<body>
<s:form action="view_submitted_adjust">
                            ...
                            <table width="100%" border="0" align="center" style="height:100%">
                                <tr>
                                    <td>
                                        ...
                                        <table width="100%">
                                            <tr>
                                                <td><b>View Submitted Adjust </b></td>
                                            </tr>
                                        </table>
                                        <table width="100%" class="displayTable">
                                            ...
                                            <tr class="bgGreen1 dsInfo">
                                                ...
                                                <td>
                                                    <s:textfield name="dataset" size="70"/><br>
                                            ...
                                            <tr class="bgGreen1">
                                                ...
                                                <%--Edit By Tum 2014 End--%>

                                            <tr class="bgGreen2">
                                                <td align="right">Adjust Approve : </td>
                                                <td>
                                                    <s:radio list="#{'':'All','UNA':'Waiting','APP':'Approved','REJ':'Rejected'}" name="adjustApprove" id="adjustApprove"/>
                                                </td>
                                            </tr>
                                            <tr class="bgGreen1">
                                        ...
                                        <display:table name="resultList2" id="row" export="false" class="displayTag" requestURI="view_submitted_adjust.action" style="width:100%;text-align:center" pagesize="<%=Constants.Pagination.DEFAULT_PAGE_SIZE%>" partialList="true" size="${requestScope.totalRecord}">
                                            <display:column title="Dataset Date" property="dsDate" format="{0,date,dd MMM yyyy}" style="text-align:center;width:10%" headerClass="headerTable" sortable="true"/>
                                            <display:column title="Title" style="text-align:left;width:10%" headerClass="headerTable" sortable="true">
                                                <a href="javascript:" onclick="detail('${row.id}','_blank')" title="Send to : ${row.exportTo}">
                                                        ${row.dsTitle2.name}
                                                </a>

                                            </display:column>
                                            <display:column title="SubTitle" style="text-align:left;width:10%" headerClass="headerTable" sortable="true">
                                                <a href="javascript:" onclick="detail('${row.id}')">
                                                    ${row.dsTitle3.name}
                                                </a>
                                            </display:column>
                                            <display:column title="Version" style="text-align:left;width:8%" headerClass="headerTable" sortable="true">
                                                <a href="javascript:" onclick="detail('${row.id}')">
                                                    ${row.dsVersion.version}
                                                </a>
                                            </display:column>
                                            <display:column title="BOT Status" style="text-align:left;width:5%" headerClass="headerTable" sortable="true">
                                                <a href="javascript:" onclick="popupBotStatus('${row.id}')">
                                                        ${row.statusBot}
                                                    <%--${row.description}--%>
                                                </a>
                                            </display:column>
                                            <display:column title="Status" style="text-align:center;width:5%" headerClass="headerTable" sortable="true">
                                                <c:if test="${not row.enable and row.status eq 'PCS'}">
                                                    <s>${row.status}</s>
                                                </c:if>
                                            ...
                                            <display:column title="Adjust Approve" style="text-align:center;width:5%" headerClass="headerTable" sortable="true">
                                                <a href="javascript:" onclick="detail('${row.id}')">
                                                    <c:choose>
                                                        <c:when test="${row.adjustApproveDate eq null and row.adjustRejectDate eq null}">
                                                            Waiting
                                                        </c:when>
                                                        <c:when test="${row.adjustApproveDate ne null}">
                                                            Approved
                                                        </c:when>
                                                        <c:when test="${row.adjustRejectDate ne null}">
                                                            Rejected
                                                        </c:when>
                                                    </c:choose>
                                                </a>
                                            ...
                                            <display:column title="Report Approve" headerClass="headerTable" style="width:5%;text-align:center" sortable="true">
                                                <a href="javascript:" onclick="detail('${row.id}')">
                                                    <c:choose>
                                                        <c:when test="${row.reportApproveDate eq null and row.reportRejectDate eq null}">
                                                            Waiting
                                                        </c:when>
                                                        <c:when test="${row.reportApproveDate ne null}">
                                                            Approved
                                                        </c:when>
                                                        <c:when test="${row.reportRejectDate ne null}">
                                                            Rejected
                                                        </c:when>
                                                    </c:choose>
                                                </a>
