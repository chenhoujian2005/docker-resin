...
<body>
<s:form action="file_transfer_detail_bot" validate="false">
                        ...
                        <td align="left">
                          <display:table name="fileUploadTransferLogs" id="row" export="false" class="displayTag" pagesize="15" requestURI="file_transfer_detail.action" style="width:100%;">
                            <display:column title="Date" headerClass="headerTable" style="width:15%;text-align:center" format="{0,date,dd MMM yyyy}">
                              <fmt:formatDate value="${row.createDateBot}" pattern="dd MMM yyyy"/>
                            </display:column>
                            <display:column title="Time" headerClass="headerTable" style="width:12%;text-align:center" format="{0,date,HH:mm:ss}">
                              <fmt:formatDate value="${row.createDateBot}" pattern="HH:mm:ss"/>
                            </display:column>
                            <display:column title="BOT Status" headerClass="headerTable" style="width:25%;text-align:left">
                              <c:choose>
                                <c:when test="${row.botStatus eq 'NTS'}">Not Send</c:when>
                                <c:when test="${row.botStatus eq 'REC'}">Received</c:when>
                                <c:when test="${row.botStatus eq 'PKB'}">Passed Key Basic Validation</c:when>
                                <c:when test="${row.botStatus eq 'FKB'}">Failed Key Basic Validation</c:when>
                                <c:when test="${row.botStatus eq 'PCX'}">Passed Complex Validation</c:when>
                                <c:when test="${row.botStatus eq 'FCX'}">Failed Complex Validation</c:when>
                                <c:when test="${row.botStatus eq 'PCS'}">Passed Cross Validation</c:when>
                                <c:when test="${row.botStatus eq 'WRS'}">Waiting For Related Sets</c:when>
                                <c:when test="${row.botStatus eq 'FCS'}">Failed Cross Validation</c:when>
                                <c:when test="${row.botStatus eq 'CAN'}">Cancelled</c:when>
                                <c:when test="${row.botStatus eq 'UNA'}">Unapproved</c:when>
                                <c:when test="${row.botStatus eq 'APP'}">Adjust Approve</c:when>
                                <c:when test="${row.botStatus eq 'REJ'}">Adjust Reject</c:when>
                                <c:when test="${row.botStatus eq 'RAP'}">Report Approve</c:when>
                                <c:when test="${row.botStatus eq 'RRJ'}">Report Reject</c:when>

                                <c:when test="${row.botStatus eq 'UPD'}">Start Upload</c:when>
                                <c:when test="${row.botStatus eq 'SBV'}">Start Basic Validation</c:when>
                                <c:when test="${row.botStatus eq 'SCP'}">Start Complex Validation</c:when>
                                <c:when test="${row.botStatus eq 'SCS'}">Start Cross Validation</c:when>
                              </c:choose>
                            </display:column>
